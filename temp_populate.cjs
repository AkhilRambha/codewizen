...
function generateComponent(componentName, category) {
...
const ${componentName} = () => {
  return (
    <>
      <PageHero 
        title="${title}" 
        description="Discover comprehensive details and insights about our ${title.toLowerCase()} program."
        breadcrumbs={[
          { label: "${category}", link: "/${category.toLowerCase()}" },
          { label: "${title}" }
        ]}
      />
      ...
      <section className="generic-page-section" style={{ padding: '80px 20px', maxWidth: '1200px', margin: '0 auto' }}>
        <div data-aos="fade-up">
          <h2 style={{ color: '#112255', marginBottom: '20px', fontSize: '32px' }}>Overview of ${title}</h2>
          <p style={{ color: '#475569', fontSize: '18px', lineHeight: '1.8', marginBottom: '30px' }}>
            Welcome to our ${title} page. We are committed to providing top-tier education and practical training. 
...
function processDirectory(directory, category) {
  ...
  files.forEach(file => {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      const content = generateComponent(componentName, category);
